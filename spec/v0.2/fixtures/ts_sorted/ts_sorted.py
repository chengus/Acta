...
# ///
"""Generate and validate the `TS_SORTED` block-flag Acta v0.2 fixture."""
...
def validate_ts_sorted(data: bytes) -> None:
    """Apply the format_v0.2.md section 8 `TS_SORTED` validation rules."""
...
def render_markdown(fixture: bytes, output_name: str) -> str:
    ...
    return f"""# `TS_SORTED` block-flag Acta v0.2 fixture
...
[`format_v0.2.md`](../../format_v0.2.md) section 8. The file contains one
...
def main() -> None:
    ...
    if not args.output and not args.markdown_output:
        print(
            f"Acta v0.2 TS_SORTED fixture passed: {len(fixture)} bytes, "
            f"sha256={digest}"
        )
