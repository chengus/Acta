# Acta file format v0.2

Status: **Release Candidate 1 (RC1)**, published 2026-07-24.

The v0.2 binary layout is frozen for compatibility testing. Files conforming
to this release candidate use format version `(0, 2)` in the file prologue.
...
interpretation will use a new format version. Implementations claiming v0.2
RC1 support MUST read the checked-in v0.2 compatibility fixtures.
...
- A **primary timestamp column** is an optional, non-nullable `timestamp64` or
  `date32` column selected as the file's time index. When it is absent,
  time-range pruning and timestamp-order claims are unavailable.
...
data. Version 0.2 defines:

- one fixed schema per file;
- an optional primary timestamp column for native time indexing;
...
Version 0.2 assigns meaning to a zero primary timestamp column ID, which was
invalid in v0.1. A v0.1-only reader MUST reject a v0.2 file as unsupported
rather than attempt to interpret it. An implementation may support both minor
versions, but MUST apply the rules belonging to the version declared by each
file.
...
Feature flag bit 0 is `ROW_IDS`; all other bits are zero in v0.2. A v0.2 reader
MUST accept format version `(0, 2)` and MUST reject every other version as
unsupported. It MUST reject unknown feature bits, a bad CRC, or a schema offset
other than 64 as invalid for v0.2. The file ID is identity metadata, not a
...
| 16 | 4 | schema flags, zero in v0.2 |
| 20 | 4 | reserved |

Its payload is a sequence of `column count` column descriptors. The column
count MUST be nonzero. Each descriptor begins on an eight-byte boundary.
...
sentinel and never identifies a descriptor. A nonzero primary timestamp ID
MUST match a non-nullable column of type `timestamp64` or `date32`. A `date32`
primary column serves daily and end-of-day series whose natural time axis is a
calendar date. The selected column is called the primary timestamp column
throughout this document, and every primary-timestamp rule applies only when
it is present.

### 7.1 Writer-facing primary selection

Writer APIs expose three schema-construction choices for the primary selection:

- `primary="<column name>"` explicitly selects the named column. The name MUST
  exist, and the column MUST be a non-nullable `timestamp64` or `date32`.
- `primary="auto"` scans columns in schema order and selects the first
  `timestamp64` or `date32` column encountered. The selected column MUST be
  non-nullable; encountering a nullable candidate fails schema construction
  rather than skipping it. If no candidate exists, schema construction also
  fails.
- `primary=None` writes primary timestamp column ID zero and creates a schema
  without native time indexing.

`"auto"` is a writer convenience and has no distinct on-disk representation;
the schema frame stores only its resolved nonzero column ID. Writers MUST NOT
...
The schema ID and column count MUST match the schema frame. An empty data block
is invalid.

When a primary timestamp column is present, timestamp bounds are calculated
over its values. When the primary column has type `date32`, the bounds are its
day counts sign-extended to `int64`, and `TS_SORTED` below claims nondecreasing
day counts the same way. The primary timestamp column is non-nullable, so every
nonempty block in such a file has valid bounds.

When no primary timestamp column is present, both timestamp-bound fields MUST
be zero and `TS_SORTED` MUST be clear. A reader MUST report any violation of
these rules as corruption. The zero values are absence sentinels, not time
bounds, and readers MUST NOT use them for pruning.
...
When a primary timestamp column is present, `TS_SORTED` declares that the
block's primary timestamp values are
...
A writer MUST set `TS_SORTED` only when a primary timestamp column is present
and after verifying the property over the block's values in row order, and
SHOULD set it whenever the property holds.
...
byte-values stream. Individual values therefore have a v0.2 maximum length of
`UINT32_MAX`.
...
min/max statistics are not written in v0.2. Nulls and floating NaNs are ignored.
Boolean min and max are each stored as one byte with value zero or one.

When a primary timestamp column is present, its min/max in the block header is
the complete mandatory pruning statistic; it need not be repeated in that
column's optional statistics area. A schema without a primary timestamp has no
mandatory pruning statistic. Implementations MAY write kind-one statistics for
other numeric, decimal, timestamp, date, boolean, or fixed-binary columns.

## 12. Encoding selection

Encoding selection is per column per block. It does not alter the schema.

| Logical type | v0.2 candidates | Expected benefit |
| --- | --- | --- |
...
capped distinct-value table while buffering. When a primary timestamp column
is present, its monotonicity statistic also determines the block's `TS_SORTED`
flag. They SHOULD estimate every cheap
...
XOR/Gorilla float encoding is intentionally excluded from v0.2. The initial
...
Readers build the time-range index from block timestamp bounds only when the
schema has a primary timestamp column. Without one, timestamp-range filtering,
timestamp-based block pruning, and timestamp ordering are unavailable; readers
scan blocks in file order. Implicit row IDs, when enabled, remain available for
stable positional identity but are not timestamps.
