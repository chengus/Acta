...
```bash
uv run benchmarks/0_taxi_data/encoding_study.py \
...
uv run benchmarks/0_taxi_data/encoding_study.py \
  ...
  --block-rows 65536 \
  --blocks 4 \
  --json-output benchmarks/0_taxi_data/results/nyc_taxi_65536.json \
  --markdown-output benchmarks/0_taxi_data/results/nyc_taxi_65536.md
```
